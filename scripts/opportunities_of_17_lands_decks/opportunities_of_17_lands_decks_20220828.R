...
# keep in memory copy of results in case something bad happens to the export file
...
# afterthought: mana curve by mana value count
# provides second opinion that this effect is real
...
# if re-running, might want to move this into loop above
...
# afterthought: show equivalent using opportunities instead of prop >=5
game3 <- group_by(game_fish, draft_id) %>% mutate(num_games = n()) %>% slice(1)

q3 <- quantile(game3$user_game_win_rate_bucket, probs = (0:10)/10, na.rm = TRUE)
t3 <- game3 %>%
    filter(num_games > 2 & !is.na(user_game_win_rate_bucket)) %>%
    group_by(win_rate = cut(user_game_win_rate_bucket, breaks = q3, include.lowest = TRUE)) %>%
    summarise(num_decks = n(),
              mean_fish = sum(mean_fish) / n())
separate(t3, col = win_rate, into = c("lo", "hi"), sep = ",") %>%
    mutate(lo = as.numeric(str_remove(lo, pattern = "\\[|\\(")),
           hi = as.numeric(str_remove(hi, pattern = "\\]"))) %>%
    group_by(row = seq_len(n())) %>%
    mutate(mid = sum(c(lo, hi)) / 2) %>% ungroup %>%
    select(-row) %>%
    ggplot(mapping = aes(x = mean_fish, y = mid)) +
    geom_point() +
    geom_smooth(method = "lm") +
    xlab("Opportunities") +
    ylab("Win Rate Bucket") +
    theme_bw()
ggsave("win_rate3.png", width = 5, height = 4)
